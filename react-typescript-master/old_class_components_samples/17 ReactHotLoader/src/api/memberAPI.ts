...
class MemberAPI {
  ...
  private _clone (item) {
  	return JSON.parse(JSON.stringify(item)); //return cloned copy so that the item is passed by value instead of by  
  };
  ...
  saveAuthor(member: MemberEntity) {
		...
		if (member.id != -1) {
			...
			//Cloning so copy returned is passed by value rather than by  .
